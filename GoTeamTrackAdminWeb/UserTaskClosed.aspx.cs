...
public partial class UserTaskClosed : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string path = Server.MapPath("/AllTeams/" + SessionManager.LoggedInUTCode + "/TaskClosed.cs");
            theDataSet.ReadXml(path);
            DataTable dtTask = theDataSet.Tables[0];
            string filter = "";

            filter = "UCode = '" + SessionManager.LoggedInUCode + "'";

            dtTask.DefaultView.RowFilter = filter;

            dtTask.DefaultView.Sort = "Task";
            rptrClosedTask.DataSource = dtTask;
            rptrClosedTask.DataBind();
